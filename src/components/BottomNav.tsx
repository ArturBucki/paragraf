          ...
          return (
            <Link
              ...
              className={`relative flex flex-1 flex-col items-center gap-1 py-2.5 transition ${
                active ? "text-coraldeep" : "text-inksoft"
              }`}
            >
              <Icon name={t.icon} className="h-6 w-6" />
              <span className="text-[11px] font-bold">{t.label}</span>
              {t.href === "/matches" && badge > 0 && (
                <span className="absolute right-[24%] top-1.5 grid h-4 min-w-[16px] place-items-center rounded-full bg-coral px-1 text-[10px] font-bold text-[rgb(var(--on-coral))]">
                  {badge}
                </span>
              )}
            </Link>
